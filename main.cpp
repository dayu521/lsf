...
void createNFA()
{
    ...
    input.open("/home/anarchy-lts/qtworkspace/lexical_syntax_analysis/词法分析_文法.txt");
    input>>N;
    while(N--)
    {
        ...
        if(strlen(str)>1)
        {
            mmove[ch][str[0]].set[mmove[ch][str[0]].len++]=str[1];
        }
        else
        {
            //if(str[0]=='@')        //用@表示伊布熊
                //is_final[ch]=true;
            //else
            mmove[ch][str[0]].set[mmove[ch][str[0]].len++]='Y';  //终态
        }
    }
}
void showNFA()
{
    for(int i=0;i<100;++i)
    {
        for(int j=0;j<100;++j)
        {
            ...
            {
                if(mmove[i][j].set[k]!='#')
                    cout<<char(i)<<" "<<char(j)<<" "<<k<<" "<<mmove[i][j].set[k]<<endl;
            }
        }
    }
}
...
void get_closure(NFA_set &temp)    //得到一个完整的子集
{
    for(int i=0;i<temp.len;++i)
    {
        for(int j=0;j<mmove[temp.set[i]]['@'].len;++j)
        {
            if(!Is_in_set(mmove[temp.set[i]]['@'].set[j],temp))
            {
                temp.set[temp.len++]=mmove[temp.set[i]]['@'].set[j];
            }
        }
    }
}
...
void NFA_to_DFA()
{
    ...
    while(!s.empty())
    {
        ...
        for(int i=0;i<len_final;++i)
        {
            ...
            {
                for(int k=0;k<mmove[work_set.set[j]][final[i]].len;++k)
                {
                    if(mmove[work_set.set[j]][final[i]].set[k]!='#' && mmove[work_set.set[j]][final[i]].set[k]!='Y' && !Is_in_set(mmove[work_set.set[j]][final[i]].set[k],worked_set))
                    {
                        worked_set.set[worked_set.len++]=mmove[work_set.set[j]][final[i]].set[k];
                    }
                    if(mmove[work_set.set[j]][final[i]].set[k]=='Y' && !Is_in_set(mmove[work_set.set[j]][final[i]].set[k],worked_set))
                    {
...
int main()
{
    ...
    NFA_to_DFA();
    //show();
    file_source=fopen("/home/anarchy-lts/qtworkspace/lexical_syntax_analysis/词法分析_源程序.txt","r+");
